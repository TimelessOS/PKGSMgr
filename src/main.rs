...
use manifest::{Chunk, parse_manifest, update_manifest};
...
fn clean_old_chunks(manifests_path: &Path, chunkstore_path: &Path) -> Result<u64, std::io::Error> {
    let mut freed = 0;
...
fn chunk_filename(chunk: &Chunk) -> String {
